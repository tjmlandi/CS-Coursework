...
//
// We need vertex buffers and element buffers:
//
BufferSet quadBuffers;
BufferSet boardBuffers;
BufferSet wineCylBuffers;
BufferSet wineConeBuffers;
...
GLuint pshader, tshader, gshader;
...
void createShape( int obj, Canvas &C )
{
    ...
    if( obj == OBJ_BOARD ) {
	boardBuffers.createBuffers( C );
    } else if ( obj == OBJ_WINE_CYLINDER ) {
	wineCylBuffers.createBuffers( C ); 
    } else if ( obj == OBJ_WINE_CONE ) {
	wineConeBuffers.createBuffers( C );
...
void init( void )
{
	...
	gshader = shaderSetup( "grapes.vert", "grapes.frag", &error );
	if ( !gshader ) {
		cerr << "Error setting up texture shader - " <<
			errorString(error) << endl;
		glfwTerminate();
    ...
    createShape( OBJ_BOARD, *canvas );
    createShape( OBJ_WINE_CYLINDER, *canvas );
    createShape( OBJ_WINE_CONE, *canvas );
...
void display( void )
{
    ...
    // draw it
    selectBuffers( pshader, boardBuffers );
    glDrawElements( GL_TRIANGLES, boardBuffers.numElements,
        GL_UNSIGNED_INT, (void *)0 );

    // now, draw the wine cylinder
    ...
    // draw it
    selectBuffers( pshader, wineCylBuffers );
    glDrawElements( GL_TRIANGLES, wineCylBuffers.numElements,
        GL_UNSIGNED_INT, (void *)0 );

    // now, draw the wine cone
    ...
    // draw it
    selectBuffers( pshader, wineConeBuffers );
    glDrawElements( GL_TRIANGLES, wineConeBuffers.numElements,
        GL_UNSIGNED_INT, (void *)0 );
    ...
    // draw it
    selectBuffers( gshader, grapesBuffers );
